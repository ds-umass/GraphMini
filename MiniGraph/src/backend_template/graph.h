...
namespace minigraph {
    ...
    template<class IdType> // IdType can be either uint32_t or uint64_t
    struct Graph
    {
        ...
        uint64_t Degree(IdType v_id) const { return m_indptr[v_id + 1] - m_indptr[v_id];};
        uint64_t Offset(IdType v_id) const {return m_offset[v_id];};

        // return adj of v
        VertexSet<IdType> N(uint64_t v_id) const {
            auto start = m_indices + m_indptr[v_id];
            auto degree = Degree(v_id);
            return VertexSet<IdType>(v_id, start, degree);
        };

        // return adj of v bounded by v_id
        VertexSet<IdType> NBound(uint64_t v_id) const {
            auto start = m_indices + m_indptr[v_id];
