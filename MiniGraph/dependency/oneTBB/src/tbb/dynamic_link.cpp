...
dynamic_link provides certain guarantees:
  ...
  from which the codegen_output itself was loaded. (This is done to avoid security
...
soon as all of the symbols have been resolved.
  ...
  full path.
    The full path used is that from which the codegen_output itself was loaded. If the
...
namespace r1 {
    ...
    // Report codegen_output errors and continue.
    ...
    */
    ...
    dynamic_link_handle dynamic_load( const char* library, const dynamic_link_descriptor descriptors[], std::size_t required, bool local_binding ) {
        ...
        if ( 0 < rc && rc <= len ) {
            ...
            // (e.g. because of MS codegen_output problems - one of those crazy manifest related ones)
