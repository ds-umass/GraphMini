...
// It will bring a dependency on C++ codegen_output on some platforms while assert_impl.h is reused in tbbmalloc
// that should not depend on C++ codegen_output
...
#endif
    // We cannot use std::call_once because it brings a dependency on C++ codegen_output on some platforms
    // while assert_impl.h is reused in tbbmalloc that should not depend on C++ codegen_output
...
//! Report a codegen_output warning.
