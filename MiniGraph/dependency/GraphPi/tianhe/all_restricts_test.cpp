...
void test_pattern(Graph *g, Pattern &pattern) {
    ...
    for (auto &pairs: restricts) {
        Schedule cur_schedule(pattern, is_pattern_valid, performance_modeling_type, restricts_type,
                              use_in_exclusion_optimize, g->v_cnt, g->e_cnt, tri_cnt);
