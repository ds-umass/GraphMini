...
void test_pattern(Graph *g, PatternType type) {
    ...
    Schedule schedule_our(pattern, is_pattern_valid, performance_modeling_type, use_in_exclusion_optimize, g->v_cnt,
                          g->e_cnt);
    ASSERT_EQ(is_pattern_valid, true);

    performance_modeling_type = 2;
    Schedule schedule_gz(pattern, is_pattern_valid, performance_modeling_type, use_in_exclusion_optimize, g->v_cnt,
                         g->e_cnt);
    ASSERT_EQ(is_pattern_valid, true);
