...
void test_pattern(Graph *g, Pattern &pattern) {
    ...
    Schedule schedule(pattern, is_pattern_valid, performance_modeling_type, use_in_exclusion_optimize, g->v_cnt,
                      g->e_cnt, tri_cnt);
    ...
    for (auto &pairs: restricts) {
        Schedule cur_schedule(pattern, is_pattern_valid, performance_modeling_type, use_in_exclusion_optimize, g->v_cnt,
                              g->e_cnt, tri_cnt);
