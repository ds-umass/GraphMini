...
public:
  ...
  Graph(std::string prefix) {//: scale_accesses(30, 0) {
    ...
    //if((pos = prefix.rfind("_b")) != std::string::npos && pos == prefix.size()-2) {
      ...
      int vid_size;
      vidType max_degree;
      //max degree is grabbed out because a version is in this file, but it is not the one used
      
      f_meta >> n_vertices >> n_edges >> vid_size >> max_degree;
      assert(sizeof(vidType) == vid_size);
      f_meta.close();
      
      std::ifstream f_labelmeta((prefix + ".meta.label").c_str());
      //max degree represents the maximum size of a vertex neighbor list, which are labelled here,
      //so we used the labelled one
      f_labelmeta >> n_labels >> max_degree;
      // std::cout<<n_labels<<" is the number of labels"<<std::endl;
      // std::cout<<max_degree<<" is the max degree"<<std::endl;
      
      VertexSet::MAX_DEGREE = std::max(max_degree, VertexSet::MAX_DEGREE);
      
      f_labelmeta.close();
      read_file(prefix + ".vertexset.label.bin", vertices, n_vertices * n_labels + 1);
      read_file(prefix + ".edge.label.bin", edges, n_edges);
      //below was replaced with the below, shouldn't actually need to check label of vertices
      //read_file(prefix + ".label.bin", L, n_vertices);
      read_file(prefix + ".label.vertices.bin",labelvertices, n_vertices);
      read_file(prefix + ".label.indices.bin",labellists,n_labels+1);
      //} else {
  ...
  VertexSet L(labelType label){
    assert(label >= 0);
    assert(label < n_labels);
    vidType begin = labellists[label], end = labellists[label+1];
    assert(begin >= 0);
    assert(begin <= end);
    assert(end<=n_vertices);
    //id of the vertexset returned shouldn't matter, but w/e
    return VertexSet(labelvertices+begin,end-begin,label);
  }
  VertexSet N(vidType vid, labelType label) {
    assert(vid >= 0);
    assert(vid < n_vertices);
    uint64_t begin = vertices[vid*n_labels+label], end = vertices[vid*n_labels+label+1];
    //scale_accesses.at((int)ceil(log2(end-begin)))++;
    assert(begin >= 0);
    if(begin > end) {
      fprintf(stderr, "vertex %u with label %d bounds error: [%lu, %lu)\n", vid, label, begin, end);
      exit(1);
    }
    assert(end <= n_edges);
